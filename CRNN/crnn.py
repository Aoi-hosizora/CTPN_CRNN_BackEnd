...
GPU = False # use GPU
pwd = os.getcwd()
if LSTMFLAG: # find model
    ocrModel  = os.path.join(pwd, "./CRNN/models", 'ocr-lstm.pth')
else:
    ocrModel  = os.path.join(pwd, "./CRNN/models", 'ocr-dense.pth')
...
def getRegion(img, points):
    """
    截取区域
    """
    print(points)
    return curregion.cut_img(img, *points)
...
def crnnSource():
    ...
    # load params
    print('loading pretrained model from %s' % ocrModel)
    model.load_state_dict(modelWeights)

    return model,converter

def crnnOcr(image, model, converter):
    """
    crnn模型，ocr识别
    image = $img.convert("L")
    $img := Image.fromarray(cv2.imread($path))
 
    @param `image` `model` `converter`
    """
...
def crnnRegionsOcr(img: np.ndarray, dict):
    '''
    ...
    for idx, frame in enumerate(dict['frames']):
        rgn = getRegion(img, [[ptns['x'], ptns['y']] for ptns in frame['points']])
        partImg = Image.fromarray(rgn)
        dict['frames'][idx]['ocr'] = crnnOcr(partImg.convert('L'), model, converter)
    return dict
